...
```
git submodule foreach 'git checkout stable-3.0 && git pull || git checkout stable-2.16 && git pull'
git submodule foreach 'git checkout master && git pull'
git submodule foreach 'git merge stable-3.0 --no-commit --no-ff || git merge stable-2.16 --no-ff || echo no merge'
