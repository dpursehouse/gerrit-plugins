...
```
git submodule foreach 'git fetch && git checkout -q origin/stable-2.15'
...
git submodule foreach 'git push origin HEAD:refs/for/stable-2.15 || echo not pushed'
git submodule foreach 'git reset origin/stable-2.15'
```
...
git submodule foreach 'git checkout -q origin/stable-2.15'
```
